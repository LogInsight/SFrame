...
template<>
struct hash<pair<graphlab::flexible_type, graphlab::flexible_type>> {
  size_t operator()(const pair<graphlab::flexible_type, graphlab::flexible_type>& x) const {
    return graphlab::hash64_combine(x.first.hash(), x.second.hash());
  }
};
}
...
void sgraph::commit_edge_buffer(size_t groupa,
  ...
  // for (size_t i = 0; i < vid_buffer.size(); ++i) {
    ...
    if (old_vids.empty()) {
      ...
      graphlab::copy_if(raw_id_sarray,
                        new_raw_id_sarray,
